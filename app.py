...
def llm_and_prompt(prompt_text, user_name=None, client_name=None, experience_year=None, uploaded_file=None):
    file_content_string = ""

    # Read the uploaded file content
    if uploaded_file is not None:
        if uploaded_file.type == "application/pdf":
            file_content_string = read_pdf(uploaded_file)
        elif uploaded_file.type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
            file_content_string = read_docx(uploaded_file)
    ...
    prompt_template = PromptTemplate(
        ...
        Note special emphises on uploaded_file content, if the uploaded_file content is present, then analyze its details and if the content is related to the
        job description then add those details in the proposal with the github link of repocitory present in the file according the description/rules provided above.

        {prompt_text}
        this is user name {user_name}
        this is client's name {client_name}
        this is years of experiance {experience_year}
        this is the uploaded_file content {file_content_string}""",
        input_variables=["prompt_text", "user_name", "client_name", "experience_year", "file_content_string"]
    )
